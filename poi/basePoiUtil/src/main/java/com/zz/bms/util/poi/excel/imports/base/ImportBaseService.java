 ...
 */
package com.zz.bms.util.poi.excel.imports.base;
...
import com.zz.bms.util.poi.excel.annotation.ExcelCollection;
import com.zz.bms.util.poi.excel.annotation.ExcelVerify;
import com.zz.bms.util.poi.excel.entity.ImportParams;
import com.zz.bms.util.poi.excel.entity.params.ExcelCollectionParams;
import com.zz.bms.util.poi.excel.entity.params.ExcelImportEntity;
import com.zz.bms.util.poi.excel.entity.params.ExcelVerifyEntity;
...
public class ImportBaseService {
    ...
    public void addEntityToMap(String targetId, Field field, ExcelImportEntity excelEntity,
    	...
    	this.addEntityToMap(targetId, field, excelEntity, pojoClass, getMethods, temp, false);
    }
    public void addEntityToMap(String targetId, Field field, ExcelImportEntity excelEntity,
                               Class<?> pojoClass, List<Method> getMethods,
                               Map<String, ExcelImportEntity> temp,boolean isDynamic) throws Exception {
    ...
    public void getAllExcelField(String targetId, Field[] fields,
        ...
        for (int i = 0; i < fields.length; i++) {
            ...
            if (PoiPublicUtil.isCollection(field.getType())) {
                // 集合对象设置属性
                ExcelCollection annotaton1 = field.getAnnotation(ExcelCollection.class);
                ...
                getExcelFieldList(targetId, PoiPublicUtil.getClassFields(clz), clz, temp, null,annotaton1.isDynamic());
                collection.setExcelParams(temp);
                collection.setExcelName(annotaton1.name());
    ...
    public void getExcelFieldList(String targetId, Field[] fields, Class<?> pojoClass,
    	...
    	this.getExcelFieldList(targetId, fields, pojoClass, temp, getMethods, false);
    }

    public void getExcelFieldList(String targetId, Field[] fields, Class<?> pojoClass,
           Map<String, ExcelImportEntity> temp, List<Method> getMethods,boolean isDynamic)throws Exception {
        ExcelImportEntity excelEntity = null;
        for (int i = 0; i < fields.length; i++) {
            Field field = fields[i];
            if (PoiPublicUtil.isNotUserExcelUserThis(null, field, targetId)) {
            ...
            if (PoiPublicUtil.isJavaClass(field)) {
                addEntityToMap(targetId, field, excelEntity, pojoClass, getMethods, temp,isDynamic);
            } else {
                List<Method> newMethods = new ArrayList<Method>();
                if (getMethods != null) {
                    newMethods.addAll(getMethods);
