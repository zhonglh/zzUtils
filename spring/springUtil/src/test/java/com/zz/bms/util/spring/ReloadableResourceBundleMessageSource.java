...
public class ReloadableResourceBundleMessageSource extends AbstractMessageSource implements ResourceLoaderAware {
    ...
    protected PropertiesHolder getMergedProperties(Locale locale){
        synchronized (this.cachedMergedProperties) {
            ...
            if (mergedHolder.getProperties () == null || mergedHolder.getProperties ().size () == 0) {
                if (!locale.equals (Locale.getDefault ())) {
                    for (int i = this.basenames.length - 1; i >= 0; i--) {
                        List filenames = calculateAllFilenames(this.basenames[i], Locale.getDefault());
                        for (int j = filenames.size() - 1; j >= 0; j--) {
                            String filename = (String) filenames.get(j);
                            PropertiesHolder propHolder = getProperties(filename);
                            if (propHolder.getProperties() != null) {
            ...
            if (resources != null && resources.length > 0) {
                ...
                for ( Resource resource : resources ) {
                    if (resource.exists ()) {
                        ...
                        if (this.cacheMillis >= 0) {
                            // Last-modified timestamp of file will just be read
                            // if caching with timeout.
                            try {
                                ...
                                // Probably a class path resource: cache it
                                // forever.
                                if (logger.isDebugEnabled ()) {
                ...
                // Resource does not exist.
                if (logger.isDebugEnabled ()) {
                    logger.debug ("No properties files found for [" + filename + "] - neither plain properties nor XML");
                }
