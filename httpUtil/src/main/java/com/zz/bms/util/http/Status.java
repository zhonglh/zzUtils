...
public final class Status {
    ...
    /**
     * This interim response (the client has to wait for the final response) is used to inform the client that the initial part of the request has been received and has not yet been rejected or
     ...
     * 
     * @see <a href="http://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html#sec14.46">HTTP RFC - 14.46 Warning</a>
     */
    ...
    /**
     * This interim response is used to inform the client that the server has accepted the complete request, but has not yet completed it since the server has a reasonable expectation that the request
    ...
    /**
     * The requested resource resides temporarily under a different URI which should not be used for future requests by the client (use status codes 303 or 307 instead since this status has been
     * manifestly misused).
     * 
    ...
    /**
     * Check if the provided reason phrase of the status contains forbidden characters such as CR and LF. An IllegalArgumentException is thrown in this case.
     * 
    ...
    private static String checkReasonPhrase(String reasonPhrase){
        if (reasonPhrase != null) {
            if (reasonPhrase.contains ("\n") || reasonPhrase.contains ("\r")) { throw new IllegalArgumentException ("Reason phrase of the status must not contain CR or LF characters."); }
        }
     ...
     *            The URI of the specification describing the method.
     */
    public Status(int code, final String reasonPhrase, final String description, final String uri) {
    ...
    public Status(Status status, Throwable throwable, String description) {
        this (status.getCode (), (throwable == null) ? status.getThrowable () : throwable, status.getReasonPhrase (), (description == null) ? status.getDescription () : description, status.getUri ());
    }
    ...
    /**
     * Returns the description. This value is typically used by the {@link StatusService} to build a meaningful description of an error via a response entity.
     * 
    ...
    /**
     * Indicates if an error is recoverable, meaning that simply retrying after a delay could result in a success. Tests {@link #isConnectorError()} and if the status is
     * {@link #CLIENT_ERROR_REQUEST_TIMEOUT} or {@link #SERVER_ERROR_GATEWAY_TIMEOUT} or {@link #SERVER_ERROR_SERVICE_UNAVAILABLE}.
     * 
     * @return True if the error is recoverable.
     */
    public boolean isRecoverableError(){
        return isConnectorError () || equals (Status.CLIENT_ERROR_REQUEST_TIMEOUT) || equals (Status.SERVER_ERROR_GATEWAY_TIMEOUT) || equals (Status.SERVER_ERROR_SERVICE_UNAVAILABLE);
    }
